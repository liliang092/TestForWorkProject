...
@property (nonatomic, strong, readonly) UIImageView * spinnerImageView;
@property (nonatomic) BOOL showImageView;
...
@synthesize overlayWindow, hudView, maskType, fadeOutTimer, stringLabel, imageView, spinnerView, visibleKeyboardHeight,showImageView,spinnerImageView;
...
+ (void)showNotIconWithSuccess:(NSString*)status
{
    [[SVProgressHUD sharedView] setShowImageView:NO];
    [SVProgressHUD showSuccessWithStatus:status];

}
+ (void)showNotIconWithError:(NSString*)status
{
    [[SVProgressHUD sharedView] setShowImageView:NO];
...
+ (void)showSuccessWithStatus:(NSString *)string {
    [[SVProgressHUD sharedView] setShowImageView:YES];
...
- (void)setStatus:(NSString *)string {
    ...
    if(string) {
        CGSize stringSize = [string sizeWithFont:self.stringLabel.font bySize:CGSizeMake(200, 300)];
        ...
        if (self.showImageView) {
            ...
            if (self.spinnerView.isAnimating) {
                labelY = (hudHeight-stringHeight)-10;
            }else{
                labelY = (hudHeight-stringHeight)/2;
            }
        }
        
        if(hudHeight > 100) {
            labelRect = CGRectMake(12, labelY, hudWidth, stringHeight);
            hudWidth+=24;
        } else {
            hudWidth+=24;  
            labelRect = CGRectMake(0, labelY, hudWidth, stringHeight);
        }
    }
	...
	if(string){
//        self.spinnerImageView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
		self.spinnerView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
    }
	else{
//		self.spinnerImageView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, ceil(self.hudView.bounds.size.height/2)+0.5);
...
//        self.imageView.hidden = YES;
        ...
        self.hudView.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.8];
...
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error {
	[self dismissWithStatus:string error:error afterDelay:2.0];
}
...
- (UIImageView *)spinnerImageView {
    if (spinnerImageView == nil){
        spinnerImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
        NSMutableArray* tempArray = [NSMutableArray allocInstance];
        
        for (int i=0; i<8; i++) {
            UIImage* tempImage = [UIImage imageNamed:[NSString stringWithFormat:@"%d.png",i+1]];
            [tempArray addObject:tempImage];
        }
        
        [spinnerImageView setAnimationImages:tempArray];
        [spinnerImageView setAnimationRepeatCount:0];
        [spinnerImageView setAnimationDuration:1];
        [spinnerImageView stopAnimating];
    }
    if(!spinnerImageView.superview)
        [self.hudView addSubview:spinnerImageView];
